(function ($) {
   ...
   *  - data
   */
  var LaravelSQLQueriesWidget = (PhpDebugBar.Widgets.LaravelSQLQueriesWidget =
    PhpDebugBar.Widget.extend({
      ...
      onFilterClick: function (el) {
        ...
        this.$list.$el
          .find("li[connection=" + $(el).attr("rel") + "]")
          .toggle();
      ...
      render: function () {
        ...
        this.$list = new PhpDebugBar.Widgets.ListWidget({
          itemRenderer: function (li, stmt) {
            ...
            var table = $(
              '<table><tr><th colspan="2">Metadata</th></tr></table>'
            )
            ...
            if (stmt.bindings && stmt.bindings.length) {
              table.append(function () {
                var icon = "thumb-tack";
                var $icon =
                  '<i class="phpdebugbar-fa phpdebugbar-fa-' +
                  icon +
                  ' phpdebugbar-text-muted"></i>';
                var $name = $("<td />")
                  .addClass(csscls("name"))
                  .html("Bindings " + $icon);
                ...
                var $bindings = new PhpDebugBar.Widgets.ListWidget({
                  itemRenderer: function (li, binding) {
                    var $index = $span.clone().text(index++ + ".");
                    li.append($index, "&nbsp;", binding)
                      .removeClass(csscls("list-item"))
                      .addClass(csscls("table-list-item"));
                  },
                });
        ...
        this.bindAttr("data", function (data) {
          ...
          for (
            var sql = {}, duplicate = 0, i = 0;
            i < data.statements.length;
            i++
          ) {
            if (data.statements[i].type === "query") {
              stmt = data.statements[i].sql;
              if (
                data.statements[i].bindings &&
                data.statements[i].bindings.length
              ) {
