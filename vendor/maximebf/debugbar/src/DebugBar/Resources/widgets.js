...
(function ($) {
   ...
   * @return {String}
   */
  var renderValue = (PhpDebugBar.Widgets.renderValue = function (
    value,
    prettify
  ) {
   ...
   *  - data
   */
  var VariableListWidget = (PhpDebugBar.Widgets.VariableListWidget =
    KVListWidget.extend({
   ...
   *  - data
   */
  var HtmlVariableListWidget = (PhpDebugBar.Widgets.HtmlVariableListWidget =
    KVListWidget.extend({
   ...
   *  - data
   */
  var IFrameWidget = (PhpDebugBar.Widgets.IFrameWidget =
   ...
   *  - data
   */
  var MessagesWidget = (PhpDebugBar.Widgets.MessagesWidget =
    PhpDebugBar.Widget.extend({
      ...
      render: function () {
        ...
        this.$toolbar = $(
          '<div><i class="phpdebugbar-fa phpdebugbar-fa-search"></i></div>'
        )
        ...
        this.bindAttr(["exclude", "search"], function () {
          ...
          for (var i = 0; i < data.length; i++) {
            var message = caseless
              ? data[i].message.toLowerCase()
              : data[i].message;
      ...
      onFilterClick: function (el) {
        ...
        var excludedLabels = [];
        this.$toolbar
          .find(csscls(".filter") + csscls(".excluded"))
          .each(function () {
   ...
   *  - data
   */
  var TimelineWidget = (PhpDebugBar.Widgets.TimelineWidget =
    PhpDebugBar.Widget.extend({
      tagName: "ul",
          ...
          if (data.measures) {
            ...
            for (var i = 0; i < data.measures.length; i++) {
              ...
              var m = $("<div />").addClass(csscls("measure")),
                li = $("<li />"),
                left = ((measure.relative_start * 100) / data.duration).toFixed(
                  2
                ),
                width = Math.min(
                  ((measure.duration * 100) / data.duration).toFixed(2),
                  100 - left
                );
              ...
              if (measure.params && !$.isEmptyObject(measure.params)) {
                var table = $(
                  '<table><tr><th colspan="2">Params</th></tr></table>'
                )
            ...
            $.each(aggregate, function (i, aggregate) {
              width = Math.min(
                ((aggregate.data.duration * 100) / data.duration).toFixed(2),
                100
              );
   ...
   *  - data
   */
  var ExceptionsWidget = (PhpDebugBar.Widgets.ExceptionsWidget =
    PhpDebugBar.Widget.extend({
